...
namespace DbcLib.Excel.Writer
{
    ...
    class Attributes
    {
        private List<ObjAttributeValue> av =
            new List<ObjAttributeValue>();

        public ObjAttributeValue this[string name]
        {
            get
            {
                return av.Find(a => a.AttributeName == name);
            }
        }

        public void Add(ObjAttributeValue a)
        {
            av.Add(a);
        }
    }
    ...
    class DbcQuery
    {
        ...
        public DbcQuery(Model.DBC dbc)
        {
            ...
            {
                if (av.ObjType == Keyword.MESSAGES)
                {
